...
OUTPUT_EXCEL = "Final_Self_Exclusion_Registry.xlsx"
# ==========================================

print("📥 Initializing EasyOCR Engine...")
reader = easyocr.Reader(['en'], gpu=False) 

def extract_perfect_data(file_path, file_name):
    """
    Directly extracts the unseparated full name and the 13-digit identity number
    from the exact layout structure provided.
    """
    ...
    try:
        ...
        # Look specifically for the "ID No" marker or sequence and extract the numbers following it
        ...
        print(f"⚠️ Error parsing file contents for {file_name}: {e}")
...
def save_styled_excel(dataframe, output_path):
    """Generates a professionally formatted corporate tracking workbook."""
    ...
    for row_idx, row_data in enumerate(dataframe.values, start=4):
        for col_idx, value in enumerate(row_data, start=1):
            ...
            if col_idx == 3:
                ...
                cell.number_format = '@'  # Keeps the leading zeros intact for SA IDs
            else:
...
# ==========================================
# 🚀 CORE BATCH PROCESSING EXECUTION
# ==========================================
if __name__ == "__main__":
    if not os.path.exists(INPUT_FOLDER):
        ...
        if not pdf_files:
            print(f"⚠ No PDF documents located inside '{INPUT_FOLDER}' folder.")
        else:
            print(f"\n📂 Located {len(pdf_files)} compliance items. Starting processing chain...\n")
            print("=" * 60)
            all_records = []
            
            for idx, file_name in enumerate(pdf_files):
                full_path = os.path.join(INPUT_FOLDER, file_name)
                record = extract_perfect_data(full_path, file_name)
                all_records.append(record)
                
                print(f"✅ [{idx+1}/{len(pdf_files)}] Processed Successfully!")
                print(f"   👤 Name: {record['Full Name']}")
                print(f"   🆔 ID  : {record['Identity Number']}")
                print("-" * 60)
                
            df = pd.DataFrame(all_records)
            save_styled_excel(df, OUTPUT_EXCEL)
            print(f"\n🎉 EXTRACtION AGENT FINISHED SUCCESSFULLY!")
            print(f"➡️ File saved locally to repository file path: {OUTPUT_EXCEL}")
